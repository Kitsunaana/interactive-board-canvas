...
export class BackgroundSizeParser {
  ...
  public constructor(private readonly _image: HTMLImageElement, private readonly _containerBounds: Rectangle) { }

  public parse(cssValue: string): Point {
  ...
  private _resolveDimensionValue(widthToken: string, heightToken: string, dimension: SizeDimension): number {
    ...
    if (BackgroundSizeParser._isPercentageValue(token)) {
      ...
      if (BackgroundSizeParser._isInvalidNumericParse(infered)) return this._image[dimension]

      return this._containerBounds[dimension] * infered
    }
  ...
  private _resolveDualValues(widthToken: string, heightToken: string): Point {
    ...
    return Point.fromSize(this._image)
  }
  ...
  private _resolveByKeyword(keyword: BackgroundSizeKeyword): Point {
    if (keyword === "auto") return Point.fromSize(this._image)

    const imagePoint = Point.fromSize(this._image)
    const boundPoint = Point.fromSize(this._containerBounds)
