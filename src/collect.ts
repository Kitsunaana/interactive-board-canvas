import { filter, fromEvent, map, merge, scan, startWith, switchMap, takeUntil, tap, withLatestFrom } from "rxjs"
import { defaultPoint } from "./new/camera/model"
import type { Camera, Point } from "./type"

type StartDraggingReturn = {
  lastPosition: Point
  panOffset: Point
  velocity: Point
  camera: Camera
...
const wheelCamera$ = wheel$.pipe(
  scan((camera, event) => _changeZoom(camera, event), INITIAL_CAMERA),
  map((camera) => ({ camera })),
)

const pan$ = pointerDown$.pipe(
  filter(canStartPan),
  withLatestFrom(wheelCamera$.pipe(startWith({ camera: INITIAL_CAMERA }))),
  map(toStartDraggingPanState),
...
export const camera$ = merge(wheelCamera$, pan$)
  .pipe(
    startWith({
...
function _dragging({ moveEvent, dragState }: { moveEvent: PointerEvent; dragState: StartDraggingReturn }) {
  return {
    ...dragState,
    camera: {
      ...dragState.camera,
...
function toStartDraggingPanState([startEvent, { camera }]: [PointerEvent, { camera: Camera }]): StartDraggingReturn {
  return {
    camera,
    panOffset: {
...
function _changeZoom(camera: Camera, event: WheelEvent) {
